...
!     *          nsteps,attnum,iAllocate,iDeallocate,jan,nrvr,nj,npick
     ...
     *          nsteps,attnum,iAllocate,iDeallocate,jan,nj
...
!     rev. 10.4.33 May.  26/21  = NK Added totUZS, API & heatdef to the wfo file
      ...
      if(wfo_spec_version_number.eq.7.0)then
        ...
        npick=npick+2 ! change this if items are added above
      endif        
...
!           Since there's no uzs for the impervious class it is used for the deficit.              
...
!     rev. 9.1.30  Nov.  08/02  - added q1, qint, drng & qlz to the wfo file
     ...
     *                         num_to_char(i)
      ...
      if(wfo_spec_version_number.eq.7.0)then
        do i=1,classcount
          if(wfo_pick(npick+15*(classcount)+i).eq.1)then
            j=j+1
            attname(j)='heatDeficit Class '//
     *                         num_to_char(i)
            attunits(j)='mm'
          endif
        end do
...
c      write(line,114) year_start,mo_start,day_start,hour_start,0,0
      write(line,114) year_now,month_now,day_now,0,0,0
