...
!               ATTRIBUTE 17 - API
                if(wfo_pick(17).eq.1)then
                  do n=1,naa
                    ii=yyy(n)
                    jj=xxx(n)
                    outwfo(jj,ii)=API(n,1)*100.0  ! same as for rff
                  end do
                  if(wfo_write_attribute_data
     *            (xcount,ycount).ne.1)then 
!                   WRITE ERROR
...
c                npick=15      ! change this if items are added above
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~` 
                ...
                do i=1,classcount
                  if(wfo_pick(npick+2*(classcount)+i).eq.1)then
                    do n=1,naa
                      ii=yyy(n)
                      jj=xxx(n)
c                      outwfo(jj,ii)=snowc(n,ii)
                      outwfo(jj,ii)=snowc(n,ii)*sca(n,ii)
                    end do
                    if(wfo_write_attribute_data
     *                (xcount,ycount).ne.1)then 
!                     WRITE ERROR
...
!     rev. 10.4.33 May.  26/21  = NK Added totUZS, API & heatdef to the wfo file
!               ATTRIBUTE 28 - heatDeficit (SNOW)
!               refer to section below "reset et sums"
                if(wfo_spec_version_number.gt.6.999)then
                  do i=1,classcount
                    if(wfo_pick(npick+15*(classcount)+i).eq.1)then
                      do n=1,naa
                        ii=yyy(n)
                        jj=xxx(n)
                        outwfo(jj,ii)=def(n,i)
                      end do
                      if(wfo_write_attribute_data
     *                  (xcount,ycount).ne.1)then 
!                       WRITE ERROR
