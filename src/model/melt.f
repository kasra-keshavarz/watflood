...
!     SAVES THE LOCAL VARIABLES FROM ONE RUN TO NEXT
	...
	REAL*4       :: time,ttime,taold,temmmp,windfactor                
      integer, dimension(:),  allocatable:: dump_class  ! for swe
...
!       Find the largest land cover class for receiving snow from redistribution         
c        allocate(dump_class(na),stat=iAllocate)
c        if(iAllocate.ne.0) STOP
c     *     'Error with allocation of dump_class in melt @ 106'
c        do n=1,naa
c            dump_class(n)=0
c            do ii=2,5
c                if(aclass(n,ii).gt.aclass(n,ii-1).and.
c     *                      aclass(n,ii-1).ne.ii.and.
c     *                      snocap(ii-1).lt.0.0)dump_class(n)=ii-1
c            end do
c        end do
c       do n=1,naa
c          write(444,*)n,dump_class(n)
c       end do
...
c            if(windsnwflg.and.windspd(ju).gt.30)then
c                windfactor=sqrt(windspd(jul_day_now)-30)/3
c            else
c                windfactor=1.0
c            endif    
...
c            if(tempv(n).gt.base(ii))then   ! Frank's
            if(tempv(n).gt.rainsnowtemp)then
              deld(n,ii)=0.0
!             TOTAL PRECIP = SNOWF+R(N,II)
              raint(n,ii)=snowf(n,ii)+r(n,ii)
              snowt(n,ii)=0
              qtot(ii)=(fm(ii)*fmadj(n))*(tempv(n)-base(ii))        !*windfactor
            elseif(tempv(n).gt.base(ii).and.
     *                              tempv(n).gt.rainsnowtemp)then  ! Frank's way
!             i.e rainsnowtemp = -999.0 - gets here for default only                
              deld(n,ii)=0.0
!             TOTAL PRECIP = SNOWF+R(N,II)
              raint(n,ii)=snowf(n,ii)+r(n,ii)
              snowt(n,ii)=0
...
!             TEMPERATURE LESS THAN OR EQUAL TO rainsnowtemp DEG.C --> ALL PRECIP IS SNOW
...
!             CHANGE IN NEG. HEAT CONT. DUE TO ENERGY EXCHANGE
              deld(n,ii)=fmn(ii)*(ati(n,ii)-tempv(n))               !*windfactor
...
!                   sca(n,ii)=0.9*oldsca(n,ii)+0.1*sca(n,ii)
                   sca(n,ii)=A7*oldsca(n,ii)+(1.0-A7)*sca(n,ii)
...
c        print*,ii,extra(ii),idump(ii),aclass(n,idump(ii))
               endif
