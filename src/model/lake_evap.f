...
!             if(lakeflg.eq.'y')then
             if(lakeflg.eq.'y'.and.ireach(n).gt.0)then  ! save time - just do lakes
...
!     qstream(n) = net precip in mm converted to cms
      ...
      do n=1,naa
!        if(ireach(n).gt.0)then    ! i.e. in a lake
        rbin=ireach(n)
...
!         when choice is false - i.e. when not on a lake OR
...
!           Temp_avg(n)=0.95*Temp_avg(n)+0.05*Temp_tot(n)/24.
...
!                 strloss(n)=strloss(n)+strloss(n)*nca_1d(n) 
!              endif
