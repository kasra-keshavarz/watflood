...
!     rev. 10.4.58 Jun.  21/22  = NK fixed nudgeflg to be off when ='n'
        do l=1,no
            if(nudgeflg.eq.'n'.and.nopt(l).eq.2)nopt(l)=1
        end do
...
c      if(rbin.eq.2)write(889,88888)n,
c     *             totaltime,year_now,month_now,day_now,hour_now,
c     *             qdwpr(rbin,jjz),qi2(n),qr(n),
c     *             qstream(n),strloss(n),qwdr(n,month_now)
c88888 format(i5,f10.0,4i5,10g15.3)
...
c      if(rbin.eq.2)write(889,88888)
c     *             n,totaltime,year_now,month_now,day_now,hour_now,
c     *             qdwpr(rbin,jjz),qi1(n),qi2(n),qr(n),
c     *             qstream(n),strloss(n),qwdr(n,month_now)
c      if(rbin.eq.2)write(889,88888)
...
!       subroutine rt_channel(div,thr,dtmin,jz,iz,time,date,n)
              call rt_channel(div,thr,dtmin,jz,iz,time,date,n) 
              lll=next(n)
...
!              if(flowsta.eq.'y'.and.ice_fctr(n).gt.0.999)then  
!              if(flowsta.eq.'y'.and.ice_fctr(n).gt.0.50)then  
...
!         write the header in rte.txt
          ...
          open(unit=56,file=filename(56),status='unknown',iostat=ios)
          if(ios.ne.0)then
              print*,'problems opening ',filename(56)(1:20)
              stop 'Program aborted in route @ 534'
          endif
!          write(56,*)'Routing state variables for debug grid # ',nnprint
!          write(56,*)'Note time interval may change - i.e. not regular'
          write(56,5551)
5551      format('   date     time,     at(hrs),       qi1,        qi2,'
     *         '         qo1,         qo2,       store1,      store2,'
     *         '          qr,        qstream,     strloss,'
     *         '    ice_fctr,     dd_thaw,    dd_ice')
          endif
	  endif

!             added Dec. 12/00 nk.
        IF(iopt99.and.n.eq.nnprint)THEN
          if(abs(float(int(time))-time).lt.0.0001)then  
!              write(56,5550)id,time,at/3600.,qi1(n),qi2(n),
...
!5550          format(i5,f10.3,11e12.3)
              ...
              write(56,5552)year_now,month_now,day_now,ii,
     *                   at/3600.,qi1(n),qi2(n),
     *                   qo1(n),qo2(n),store1(n),store2(n),
     *                   qr(n),qstream(n),strloss(n),
     *                   ice_fctr(n),dd_thaw(n),dd_ice(n)
5552          format(i4.4,'-',i2.2,'-',i2.2,2x,i2.2,':00',13(',',e12.3))
           endif
        endif
