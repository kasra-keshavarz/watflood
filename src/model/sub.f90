...
!     This take precidence over the addition & multiplication methods  
      use_nc_swe_update=.false.  ! default (just incase the file does not exist)
...
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      if(debug_output)print*,'FEWS use_nc_swe_update =',use_nc_swe_update
      if(use_nc_swe_update)then
...
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
              if(debug_output)then
                  print*,'FEWS use_nc_swe update @ 892=',use_nc_swe_update
                  if(use_nc_swe_update)print*,'@ ',yy_swe,mm_swe,dd_swe,hh_swe
              endif
          else    
              write(98,*)'Error: snow1\SWEdate.xml required but does not exist'
              write(98,*)'Error: Program aborted in sub @ 856'
              write(*,*)'Error: snow1\SWEdate.xml required but does not exist'
...
!     This is to just use a multiplier or an additive amount
      ...
      if(.not.use_nc_swe_update.and.exists)then
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!     Allocate the diversion flow output vector only
!     for max file length. Rest of allocations in lst  
!     This to divert for another domain charm run      
      wrtdiverflg=.false.
      INQUIRE(file='diver\diversion_locations.txt',exist=exists)
      if(exists)then
          wrtdiverflg=.true.
          open(unit=99,file='diver\diversion_locations.txt',status='unknown',iostat=ios)
          read(99,*)nDivOut  ! # receiving grid d/s basin
          print*,'found ',nDivOut,' locations'
          allocate(divOut_xsta(nDivOut),divOut_ysta(nDivOut),&
               divInName(nDivOut),divOutName(nDivOut),&
               divIn_xsta(nDivOut),divIn_ysta(nDivOut),&
               DivRow(nDivOut),divCol(nDivOut),Divgrid(nDivOut),&
               qdiv(nDivOut,8784),&
               stat=iAllocate)
          if(iAllocate.ne.0)STOP 'Error with allocation of diversion output in lst @ 1010'
!         read the row & col for the diversion output location          
          do l=1,nDivOut
              read(99,*)divOut_xsta(l),divOut_ysta(l),divOutName(l)
          end do
!         read the row & col for the flow source - a dummy in this case(outlet grid)        
          read(99,*)
          do l=1,nDivOut
               read(99,*)divIn_xsta(l),divIn_ysta(l),divInName(l)
          end do
!         find the row & col for the diversion output location          
          do l=1,nDivOut
              divRow(l)=int((divOut_ysta(l)-yorigin)/ydelta)+1
              divCol(l)=int((divOut_xsta(l)-xorigin)/xdelta)+1
              divGrid(l)=s(divRow(l),divCol(l))
          end do  
          read(99,*)outputDirectory
          close(unit=99,status='keep')
!          INQUIRE(file=outputDirectory,exist=exists)
!          if(.not.exists)then
!              wrtdiverflg=.false.
!              print*,'Warning:'
!              print*,'Output directory not found'
!              print*,outputDirectory
!              print*,'Continuing without writing a diversion file'
!              print*
!          else
!              print*,'write div ok'
!          endif
...
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          call header()
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!              ensimflg='n'   Taken out Mar. 5/23  nk
!              writeflg(2)   =.false.
          endif
        endif
...
!         basin/bsnm.shd
    ...
    if(debug_output)write(63,*)'reading vectorized shd file'      
...
!       unit   286 = fln(56) - wind speeed *spd.tb0
...
!       rev. 9.9.35  Oct.  20/14  - NK: Added keyword & file checks
          ...
          if(.not.exists)then
            ...
            write(*,*)'Error: ',fln(10)(1:40),' precip file not found'
            ...
            write(98,*)'Error: ',fln(10)(1:40),' precip file not found'
            ...
            stop 'Program aborted in sub @ 2 - SEE debug\warnings.txt'
          endif
...
!           ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            if(id.eq.1)then
              ...
              allocate(evap_convert(na),stat=iAllocate)
...
!             ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            if(resumflg.ne.'n')then  ! could be y or s
...
!             Added this here for FEWS  NK Jul.                
              if(iopt99.or.FLtype(6).eq.'nc')then
                open(unit=99,file='reservoir_location.xyz',status='unknown',iostat=ios)
                if(ios.ne.0)then    ! added Nov. 10/14  nk
...
!                 so the datum can be plotted
                  ...
                  if(dds_flag.eq.0)then
                    ...
                    if(lakeflg.eq.'y'.or.ruleflg)then 
                      write(*,*)'Error: NOTE: for lake evap model or lake rules,'
                      write(*,*)'Error:  ill file with lake depths is required'
                      write(98,*)'Error: NOTE: for lake evap model, ill file with'
                      write(98,*)'Error: lake depths are required'
                      write(98,*)'Error: ditto for lake rules'
          ...
          if(divertflg.eq.'y')then
            ...
            if(exists)then
              diversion=.true.
                ...
                write(*,*)'Info: Diversion file read:',fln(52)(1:40)
            else
...
!             ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            elseif(FEWSflg.eq.'n')then
              write(*,*)'Error: Found SWE file ',fln(36)(1:32)
...
!     rev. 9.8.16  Mar.  21/11  - NK: reinstate reservoir inflow error for dds
            if(i.ne.no+noresvi)then
                ...
                print*,'It is needed only for DDS'
                stop 'Program aborted in sub @ 2869'
            endif
...
!     rev. 10.4.53 Mar.  14/22  = NK Modifications for weather model diversio inflowg
!        if(modelflg.ne.'n'')then
        if(modelflg.ne.'n'.and.modelflg.ne.'d')then
...
!        windsnwflg=.false.        
!        INQUIRE(FILE=fln(56),EXIST=exists)   ! winds\yyyymmdd_spd.tb0
!        IF(exists)THEN

!            call read_tb0(286,56)
!!            do j=deltaT_temp,ycount_temp,deltaT_temp
!!              print*,j,inarray(j,1)
!!            end do
        
!        endif
!        if(.not.allocated(windspd))then
!             allocate(windspd(ycount_temp/deltaT_temp),stat=iAllocate)
!             if(iAllocate.ne.0)then
!               write(98,*)'Error: Error with allocation of windspd in sub @ 3235'
!               STOP 'Error with allocation of p in sub @ 117'
!             endif
!        endif
!        do j=deltaT_temp,ycount_temp,deltaT_temp
!!                 convert to one value / day                  
!                  windspd(j/deltaT_temp)=inarray(j,1)
!        end do
!        windsnwflg=.true.        
!        do j=1,365
!              print*,j,windspd(j)
!        end do
!        print*,'windspd in sub'
...
!        if(fln(10)(1:21).eq.'radcl\climate_pcp.r2c')climateFlg=.true.
        if(fln(6)(1:13).eq.'strfw\climate')climateFlg=.true.
!       this will allow radcl\climate_yyyy_*.r2c        
...
!     rev. 10.2.55 June  09/19  - NK Added read_swe_update.f90 for read in swe adjustment factors
!         Method 2:  This will use the XML file           snow1\swe_update.xml
          if(swe_update)then 
!             This order is with the least # checks - but fix so we find hour from sart so there needs to be just one check              
              if(year_now.eq.yy_swe.and.month_now.eq.mm_swe.and.day_now.eq.dd_swe.and.hour_now.eq.hh_swe)then
...
!                 This keeps the relative amounts wrt. land cover classes                  
                  ...
                  if(debug_output)print*,'-------------------------------'
                  if(debug_output)print*,'swe multiplied by ',swe_mult,swe_update_done,' in sub @ 3375'
                  if(debug_output)print*,'-------------------------------'
...
!                 This does not keep the relative amounts wrt. land cover classes                  
                  ...
                  swe_update_done=.true.
                  if(debug_output)print*,'-------------------------------'
                  if(debug_output)print*,swe_add,' mm added to swe',swe_update_done,' in sub @ 3398'
                  if(debug_output)print*,'-------------------------------'
              endif
              endif
          endif
...
!                         set upper limit for uzs adjustment                          
                  ...
                  write(98,*)'Info: UZS multiplier applied on ',year_now,month_now,day_now,hour_now
                  write(*,*)'Info: UZS multiplier applied on ',year_now,month_now,day_now,hour_now
                  if(uzs_mult.gt.-998..and.uzs_add.gt.-998.)then
                      ...
                      if(debug_output)write(*,*)'Info: Mult applied on ',year_now,month_now,day_now,hour_now
                  endif
...
!                         set limits for uzs adjustment                          
                  ...
                  if(debug_output)write(*,*)'Info: UZS addition applied on ',year_now,month_now,day_now,hour_now
              endif
              endif
              endif
          endif
...
!     rev. 10.4.53 Mar.  14/22  = NK Modifications for weather model diversio inflowg
          elseif(modelflg.eq.'d')then  
              
!           DO NOTHING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!           Bypass hydrological modelling              
            continue
...
!                    read(cday,1000)cdayYear,cdayMonth,cdayDay
!                    print*,cdayYear,cdayMonth,cdayDay
!c                    today=julian_day(cdayYear,cdayMonth,cdayDay)
!     rev. 10.4.31 Dec.  22/20  = NK fix climate end of year
                    today=julian_day(year_now,month_now,day_now)
...
!     rev. 10.3.08 Mar.  06/20  = NK For FEWS, add snow1\swe.nc for swe updating
                  if(use_nc_swe_update)then    ! read by read_swe_update.f90  (above)
      ...
      !                if(debug_output)print*,'charm @ ',year_now,month_now,day_now,hour_now_fews
                      if(hh_swe.eq.hour_now_fews.and.dd_swe.eq.day_now.and.mm_swe.eq.month_now.and.yy_swe.eq.year_now)then
...
!             precip has been disaggregated elsewhere                
!             ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
              call read_rain_ef('0',conv,jz+startHour,jan) !EnSim compatible r2c file
!             ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            endif
...
!          if(mod((jz-1),6).eq.0)write(666,*)tempv(925)


!     rev. 9.9.06  Jan.  08/14  - NK: Add daily differences to Harfreaves ETHarg.f
            if(dlyflg)then
                if(allocated(inarray))deallocate(inarray)     !fix <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!     rev. 9.5.56  Mar.  26/09  - NK: Fix bug with month in yearly events
           ...
           if(debug_output)write(63,*)' Gone to ice_factor'
...
!           note: ice_factor is called if icerivflg and/or icelakeflg = y
            if(iceflg.eq.'y'.and.mod(jz+23,24).eq.0)then
!           ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
              call ice_factor 
!           ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!                write(666,*)year_now,month_now,day_now,hour_now,ice_fctr(1150)
            endif
...
!     rev. 10.4.53 Mar.  14/22  = NK Modifications for weather model diversio inflowg
         if(modelflg.eq.'d')then  ! channel inflow from diversions only - no hydrological modelling
             do n=1,naa
                 qr(n)=0.0
                 qi2(n)=0.0
             end do
...
!                         write(887,88777)year_now,month_now,day_now,jz,qi2(gridrecieve(nd)),qdivert(nd,jz),&
!                          qi2(gridrecieve(nd))+qdivert(nd,jz)
!                        For FEWS (netCDF) only:                
!                        In s/r read_ts_diversion_nc qdivert = +ve 
!                        for receiving grid, -ve for source grid            
                         qi2(gridrecieve(nd))=qi2(gridrecieve(nd))+qdivert(nd,jz)
!                         write(887,88777)gridsource(nd),gridrecieve(nd)
!88777                    format(4i5,3f15.3)       
                     endif 
                  end do
              else
                  do nd=1,nodivert
                      if(divert_inbasin_flg(nd))then
!     rev. 10.5.07 Apr.  25/23  = NK Fixed diversion to prevent outflow > available storage+ inflow
                        If(gridsource(nd).eq.na)then
!                         Source is outside watershed - no restrictions                            
                          qr(gridrecieve(nd))=qr(gridrecieve(nd))+qdivert(nd,jz)
                        else  
!                       Divert only if there's eneuf storage in the source lake    
                          if(store2(gridsource(nd)).gt.qdivert(nd,jz)*3600.)then
                              qr(gridsource(nd))=qr(gridsource(nd))-qdivert(nd,jz)
                              qr(gridrecieve(nd))=qr(gridrecieve(nd))+qdivert(nd,jz)
!                          if(nd.eq.2)write(888,*)totaltime,qr(gridsource(nd)),qdivert(nd,jz),&
!                              store2(gridsource(nd)),qdivert(nd,jz)*3600.
                          else
                              qr(gridsource(nd))=0.0
                              qr(gridrecieve(nd))=0.0
                          endif
                        endif
                      endif 
                  end do
...
!         netCDFflg     if true: both read & write nc formats
!         netCDFoutflg  if true, only write nc output formats          
          if(netCDFflg.or.netCDFoutflg.eq.'y')then
            do n=1,naa
              totd1(n)=0.0
              totuzs(n)=0.0
...
!                                        +(retn(ii)-uzsfs(n,ii))*aclass(n,ii)*sca(n,ii)
                     totuzs(n)=totuzs(n)+(uzs(n,ii))*aclass(n,ii)*(1.0-sca(n,ii))&
...
!     rev. 10.4.21 Apr.  21/20  = NK Add UZS deficit to wfo file = UZS(class=classcount)
              ...
              do ii=1,classcount-3
                  if(aclass(n,ii).gt.0.0)then
                     totuzsdef(n)=totuzsdef(n)+(retn(ii)-uzs(n,ii))*aclass(n,ii)*(1.0-sca(n,ii))&
                                        +(retn(ii)-uzsfs(n,ii))*aclass(n,ii)*sca(n,ii)
                     totuzsdef(n)=amax1(0.0,totuzsdef(n))
                  endif
              end do
            end do
          endif
...
!     Note:  this is also done for updating swe above  -  this stays here for reporting in wfo file
          if(numa.eq.0.and.ensimflg.eq.'y'.or.netCDFflg.or.netCDFoutflg.eq.'y')then
...
!     rev. 10.4.49 Feb.  05/22  = NK Fixed swe (under)reporting in the watflood.wfo & snw.csv files 
!                      class_sum=class_sum+aclass(n,ii)
                      class_sum=class_sum+aclass(n,ii)      !*sca(n,ii)             ! <<<<<< = correct
                      totsnw(n)=totsnw(n)+snowc(n,ii)*aclass(n,ii)*sca(n,ii)
!                      if(n.eq.nnprint)write(555,*)totsnw(n),snowc(n,ii),aclass(n,ii),sca(n,ii),ii
                  endif
                endif
              end do
              totsnw(n)=totsnw(n)/class_sum
!              if(n.eq.nnprint)write(555,*)totsnw(n)
            end do   
          endif
...
!     rev. 10.4.52 Feb.  25/22  = NK Added swe.txt for swe on debug grid for plotting
        if(mod(int(time),24).eq.0)then
          write(64,*)time/24,totaltime,totsnw(nnprint)    
!         swe on the debug grid
        endif
...
!            call write_r2c(100,100,int(totaltime),0,frame_no5,0,8)  
            call write_r2c(100,100,int(totaltime),0,frame_no5,0,-1)  
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            endif
            endif    ! iopt99
...
!     rev. 10.5.05 Mar.  25/23  = NK Fixed reporting in wfo file
...
!     rev. 10.4.49 Feb.  05/22  = NK Fixed swe (under)reporting in the watflood.wfo & snw.csv files 
!                    class_sum=class_sum+aclass(n,ii)        
                    class_sum=class_sum+aclass(n,ii)   !*sca(n,ii)        ! changed 2022-02-05  NK <<<<<<<
...
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!                call write_2D_flow(int(totaltime/deltat),0,.false.,201)
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!               weighted average of swe                
                do n=1,naa
                  i=yyy(n)
                  j=xxx(n)
                  outarray(i,j)=totsnw(n)
...
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!               call write_2D_swe(int(totaltime/deltat),0,.false.,202)
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!               weighted average of uzs/retn                
                do n=1,naa
                  i=yyy(n)
                  j=xxx(n)
...
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!                call write_2D_uzs(int(totaltime/deltat),0,.false.,203)
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
...
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!                call write_2D_grid_runoff(int(totaltime/deltat),0,.false.,204)
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
                do n=1,naa                  ! added June 13/19 NK
                  i=yyy(n)
                  j=xxx(n)
...
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!                call write_2D_cumm_et(int(totaltime/deltat),0,.false.,205)
!               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
              endif
...
!          write(555,55500)(lake_tmp(l),l=1,noresv)
!55500     format(<noresv>F8.1)          
...
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!          call write_ts_flow(no,int(time)/deltat,.false.,99) 
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        
          fln(99)='results\CHARM_lake_levels.nc'
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!          call write_ts_lake_levels(noresv,int(time)/deltat,.false.,99) 
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          
          fln(99)='results\CHARM_lake_inflow.nc'
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!          call write_ts_lake_inflow(noresv,int(time)/deltat,.false.,99) 
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          
          fln(99)='results\CHARM_lake_outflow.nc'
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!          call write_ts_lake_outflow(noresv,int(time)/deltat,.false.,99) 
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!     rev. 10.4.47 Jan.  09/22  = NK Additional constraints on flow_reset in sub
        if(id.eq.1)then
            if(flowresetflg.eq.'y')then
                    call flowreset
            endif
        endif
...
!         close the files          
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!          call write_2D_flow(int(totaltime/deltat),int(totaltime/deltat),.true.,201)
!          call write_2D_swe(int(totaltime/deltat),int(totaltime/deltat),.true.,202)
!          call write_2D_uzs(int(totaltime/deltat),int(totaltime/deltat),.true.,203)
!          call write_2D_grid_runoff(int(totaltime/deltat),int(totaltime/deltat),.true.,204)
!          call write_2D_cumm_ET(int(totaltime/deltat),int(totaltime/deltat),.true.,205)
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
          deltat=6
          fln(99)='results\CHARM_flow_vector.nc'
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!          call write_ts_flow(no,int(time)/deltat,.true.,99) 
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          
          fln(99)='results\CHARM_lake_levels.nc'
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!          call write_ts_lake_levels(noresv,int(time)/deltat,.true.,99) 
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          
          fln(99)='results\CHARM_lake_inflow.nc'
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!         call write_ts_lake_inflow(noresv,int(time)/deltat,.true.,99) 
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          
          fln(99)='results\CHARM_lake_outflow.nc'
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!          call write_ts_lake_outflow(noresv,int(time)/deltat,.true.,99) 
!         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          write(98,*)'Info: Finished writing the .nc output files files'       
      endif
...
!     rev. 10.5.08 May   03/23  = NK Base flow Index BFI  added
      if(iopt99.and.trcflg.eq.'y')then
        INQUIRE(FILE='strfw\nudge_flags.xyz',EXIST=exists)
        IF(exists)THEN
            open(unit=99,file='strfw\nudge_flags.xyz',iostat=ios,status='unknown')
            if(ios.eq.0)then
            do l=1,no
              read(99,99100)junk,names(l)
99100         format(a47,a45)              
!              write(*,99101)l,gage(l),isoconcGWsum(l)/qo2Sum(l),names(l)(1:45)
            end do
            close(unit=99,status='keep')
            open(unit=99,file='results\BFI.txt',iostat=ios,status='unknown')
            do l=1,no
              write(99,99101)l,gage(l),isoconcGWsum(l)/qo2Sum(l),names(l)(1:45)
99101         format(i5,2x,a8,'  BFI= ',f8.2,2x,a45)
           end do
           print*,'Finished writing results\BFI.txt (BaseFlowIndex)'
           endif
           close(unit=99,status='keep')
        else
           Print*,'strfw\nudge_flags.xyz not found'
           print*,'results\BFI.txt will not be written.'
        endif
      endif  !iopt99              
...
!     rev. 10.4.26 Aug.  26/20  = NK Added Kling–Gupta efficiency (KGE) score
      ...
      if(errflg.eq.13)then
!         KGE calculated in stats.f          
          if(optim.ne.optim)then    ! check for nan
              optim=-100.0
          endif
      else
!         error calculated in dds_error.f          
          optim=-dds_error
      endif
      write(*,*)'dds_error = optim = ',optim
!     write the function value for DDS
!     for Ostrich:      
      open(unit=99,file='function_out.txt',status='unknown',iostat=ios)
      write(99,*,iostat=ios)optim
        if(ios.ne.0)then    ! added Nov. 10/14  nk
          print*
          print*,'Unable to open file function_out.txt'
          print*,'Possible cause(s):'
          print*,'file in use by another application'
          pause 'Program pause in sub.f @ 5001'
        endif
      close(unit=99,status='keep')

      !     for DDS - stand alone  - no longer needed with OstrichMPI
      if(dds_flag.eq.1)then
...
!          stop 'Program aborted in sub.f @ 4299'
        ...
        write(99,*,iostat=ios)optim
!        write(99,*)'function:,KGE'
!        write(99,*)'Obj_fn:,  ',optim
