...
!                 CHANNET --> WETLAND FLOW ONLY
              ...
              do ic=1,40   ! convergence loop - doubled # 2023-05-31
...
!     rev. 10.2.71 Nov.  18/19  - NK Bug fixes in wetland & reservoir routing
                  ...
                  if(NaNtest)then
                      warning(3)=.true.
                      qowet2(n)=0.0
                      write(98,*)'Error: wetland NaN in grid ',n,'at t= ',totaltime
                      write(98,*)wstore2(n),store2(n),hwet2(n),hcha2(n),hwet2(n)**2-hcha2(n)**2
                      write(*,*)'Error: wetland NaN in grid ',n,'at t= ',totaltime
                      write(*,*)'Outflow set =0.0'
!                      pause 'in rt_wetland @ 212'
                  endif

!                 added Jan. 6, 2023  NK
                  if(store2(n).gt.1.0)then
                      flowxa(n)=store2(n)/rl(n)
                      qo2(n)=flowxa(n)**1.67*slope(n)/chawid(n)**0.667/r2n(n)*ice_fctr(n)

                
                if(.not.qo2(n).eq.qo2(n))Print*,'wa',jx,n,ax,pool(n)
                  ...
                  if(NaNtest)then
                      qo2(n)=0.0
                      write(98,*)'Error: flow NaN in grid ',n,'at t= ',totaltime
                      write(*,*)
                      write(*,*)'Error: flow NaN in grid ',n,'at t= ',totaltime
                      write(*,*)'Chanel xsct area = ',flowxa(n),store2(n),rl(n)
                      write(*,*)'Hit ctrl C to abort; enter to continue'
!                      pause 'in rt_wetland @ 225'
                  endif

!     rev. 9.8.12  Dec.  07/11  - NK: removed 30 char limit on find filetype 
                  wt=amax1(.1,float(ic)/41.)
                  wt=amin1(1.00,wt)
...
!                     withdrawals were added to the code
                ...
                if(.not.qo2(n).eq.qo2(n))Print*,'wb',jx,n,ax,pool(n)
...
26            continue
      ...
      if(qo2(n).lt.0.000)write(98,*)'Warning: W',n,id,time,qo2(n)
                ...
                if(.not.qo2(n).eq.qo2(n))pause 55555
