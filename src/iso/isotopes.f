...
! REV 2.3 8/2016 - ISOsurface, ISOinter and ISOground merged into ISOland, fractionation in streams back (TH)
!
!************************************************************************
...
!         INITIALIZE LAKE GRID ARRAYS
     ...
     *          isoSTRconc2(n)*(store1(n)-store_dead(rbin))
	    endif
...
!         STORES THE GRID NO. AS FXN OF THE GAUGE(BSN) NO.
     ...
     *          iso2HSTRconc2(n)*(store1(n)-store_dead(rbin))
	    endif
	    if(wetland_flag(n))then
            iso2HWETevap(n)=0.0 
            iso2HWETin2(n)=0.0
...
! * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *   
        ...
        if(firstpass_local.or.year_now.gt.year_last)then
	  isoyearnum=-99
        do m=1,ninit
