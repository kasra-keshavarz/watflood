...
!     SAVES THE LOCAL VARIABLES FROM ONE RUN TO NEXT
      ...
      INTEGER :: i,j,k,l,n,ios,no_old,iAllocate,ideallocate,linecount,dl,lineno,idlast,WFruntime
      LOGICAL exists,nudge,msgFlag,ncFlg    !ncflg to be used when reading .nc format flow file
...
!     rev. 10.2.58 Jul.  17/19  - NK Remap nopt for changed flow stationlocations in FEWS
      ...
      integer   ll1,ll2
!     rev. 10.2.66 Sep.  30/26  - NK Changed gage_temp from 7 to 8 char variable
      character(8), dimension(:), allocatable :: gage_temp
...
!     rev. 9.9.35  Oct.  20/14  - NK: Added keyword & file checks
    ...
    !         call rdflow('0',date)   
              ...
              if(iopt.ge.1)print*,'Opened unit',unitNum,' filename  ',fln(flnNum)(1:40)
          else
...
!	if(iopt.eq.2)print*,'reading line ',linecount,' in read_flow_ef'
            ...
            if(line(1:1) .eq. ':')then
                ...
                if(keyword(1:KeyLen) .eq. ':endheader')then
                    ...
                    iStat = ParseFlowColumnMetaData(colHeader,keyword,keyLen,subString)
                    if(iStat .lt. 0) then
...
!     rev. 9.7.07  Sep.  05/10  - NK: increased allowed # flow stations from 128 to 512
          ...
          if(no.le.0)then  ! added Oct. 18/20 nk
            print*,'no of flow stations found =',no
            print*,'# needs for be > 0'
            print*,'Please fix flow file'
            print*
            stop 'Program aborted in read_flow_ef @ 201'
          endif
...
!     fixed  nk  Nov. 22/06
          ...
          CALL GoToStartOfData(unitNum)
            ...
            if(firstpass.eq.'y')then
              allocate(qhyd(no,nl+24),stat=iAllocate)
!             Extra 24 hours needed because month count is approx in lst              
              if(iAllocate.ne.0) STOP 'Error with allocation of qhyd arrays in in read_flow_ef @ 234'
!             this is needed herefor non-fews runs            
              allocate(flow_sta_name(no),stat=iAllocate)
              if(iAllocate.ne.0) STOP 'Error with allocation of flow_sta_name in in read_flow_ef @ 234'
            endif
...
!c    *		areasum(no),
            ...
            if(iAllocate.ne.0) STOP'Error with allocation of area5a arrays in in read_flow_ef'
!     moved to flow init
...
!     REV. 10.1.34 Jul   05/16  - NK: Added Obs. & Model mean flows to wfo file
            allocate(wfo_qhyd(na,nl+24),wfo_qsyn(na),stat=iAllocate)
            if(iAllocate.ne.0) STOP'Error with allocation of wfo_qhyd arrays in read_flow_ef'

!     rev. 10.2.16 Feb.  14/18  - NK: Added bankfull flow calculation
            allocate(bankfull(ycount,xcount),stat=iAllocate)
            if(iAllocate.ne.0) STOP'Error with allocation of bankfull array in read_flow_ef'

!     rev. 
            allocate(xNudge(no),yNudge(no),nNudge(no),gage_temp(no),&
     &               jnudge(no),iNudge(no),nopt_temp(no),stat=iAllocate)
            if(iAllocate.ne.0) STOP'Error with allocation of xNudge & yNudge in read_flow_ef'
...
!           DEALLOCATION OF ARRAYS FROM AREA10A:
               ...
               allocate(qhyd(no,nl+24),qsyn(no,nl+24),&
                qhyd_dly(no,nl/24),qsyn_dly(no,nl/24),&
!c    *			qhyd_mly(no,max(1,nl/672)),qsyn_mly(no,max(1,nl/672)),
                qhyd_mly(no,max(12,nl/672)),qsyn_mly(no,max(12,nl/672)),&
                       stat=iAllocate)
               if(iAllocate.ne.0) STOP	'Allocation Error: area5b arrays in read_flow_ef@230'
...
!                       qdwpr(noresv,nl),&
!                       stat=iAllocate)
!                if(iAllocate.ne.0) STOP'Allocation Error:  arrays in read_flow_ef@239'
!                  endif
            endif
      endif               !  firstpass
!       rev. 9.1.68  Dec.  19/04  - NK: rewrote rdflow c/w memory allocation 
...
!     rev. 10.4.39 Sep.  26/21  = NK Set nopt for hdrflg = '0' only
      if(FLtype(6).eq.'tb0'.and.hdrflg.eq.'0')then
...
!     rev. 10.2.58 Jul.  17/19  - NK Remap nopt for changed flow stationlocations in FEWS
!     rev. 10.2.66 Sep.  30/26  - NK Changed gage_temp from 7 to 8 char variable and used as flag ID
!           check the nudge locations match the flow locations
            msgFlag=.true.
!            if(iopt99)write(98,*)'      flow station locations      nudge locations '      
!            if(iopt99)write(98,*)'rch#  long     lat     iy   jx    long     lat   iN   jN   fl'
!            do l=1,no
!!             find the grid #                
...
!               if(jNudge(l).ne.jx(l).or.iNudge(l).ne.iy(l))then
               if(gage_temp(l)(1:7).ne.gage(l)(1:7))then  
                 if(msgflag)then  
                   write(98,98001)'WARNING: ',&
                    ' Nudge location for some or all stations ',&
                    ' do not match the flow station',&
                    ' location in the strfw location.'
98001               format(a9,a40,a30,a32)                   
                    msgFlag=.false.
                  endif
                endif
            end do
            close(unit=99,status='keep')
            write(98,*)'Info: Finsished reading strfw\nudge_flags.xyz'
          endif
            
          if(msgFlag)then
!         This means the order in the flag file is different from the order in the str file              
!         Reset the flags using the lat-long in the nudge_flags.xyz file
              write(98,*)'Info: Reset the flags using the lat-long in nudge_flags.xyz' 
              do l=1,no
                  nopt(l)=1
              end do
              write(98,*)'Info: nudge station # flow station #   nudgeflg'  
...
!                      if(jNudge(i).eq.jx(l).and.iNudge(i).eq.iy(l))then
                      ...
                      ll1 = LEN_TRIM(gage_temp(i))
                      ll2 = LEN_TRIM(flow_sta_name(l))
                      ll1=min(ll1,ll2)
                      if(gage_temp(i)(1:ll1).eq.flow_sta_name(l)(1:ll1))then
                          nopt(l)=nopt_temp(i)
                          if(iopt99)write(98,*)'Info: nopt ',i,l,nopt(l)
                      endif
                  end do
              end do
!            else
!              do l=1,no    ! nudge file
!                    nopt(l)=nopt_temp(l)
!              end do
            endif
            
            write(98,*)'Info: Nudged flow stations as used in CHARM:'
            write(98,*)'Info:    Flow station   Nudgeflg     long       lat'
            do l=1,no
                if(nopt(l).eq.2)write(98,*,iostat=ios)'Info: ',l,flow_sta_name(l),nopt(l),xstr(l),ystr(l)
                if(ios.ne.0)then   ! added oct. 09.19 NK
                    write(98,*)'Error: Error with the nudgeflag file'
                    write(98,*)'Error: Program aborted in read_flow @ 461'
                    stop 'Program aborted in read_flow @ 461'
                endif
            end do
            
!     rev. 10.2.58   end

            open(unit=99,file='debug\nudged_stations.xyz',status='unknown')
            do l=1,no
              if(nopt(l).eq.2)then
                write(99,*)xstr(l),ystr(l),l,flow_sta_name(l)
              endif
            end do
            close(unit=99,status='keep')
        endif    ! nudgeflg
...
!       flag the stations that will be nudged:
        ...
        do l=1,no
          if(nopt(l).eq.2)then
            write(98,*)'INFO: nudging in 1st event is risky'
            if(nopt(l).eq.2)nudge=.true.
          endif
        end do
        if(nudge)then
          write(98,*)'INFO: Flows nudged at designated flow stations as designated in event #1'
        endif
...
!         use nopt as read in this event's flow file
          ...
          if(nudge)then
            write(98,*)'INFO: Flows nudged at designated flow stations as designated in the current event'
          endif
        endif
      endif
...
!     rev. 10.2.68 Nov.  11/19  - NK New strfw\flowstation.xyz to set locations of the flow gages
        inquire(file='strfw\flowstations.xyz',EXIST=exists)          
        if(exists)then
!             Replace str file locations with those in flowstations.xyz   
!             Done for FEWS in case stations end up in the wrong grid
!             Stations can then be moved a little without haveing to mess with FEWS            
              open(unit=99,file='strfw\flowstations.xyz',status='unknown',iostat=ios)
              if(ios.ne.0)then
                write(98,*)'Error: problems opening strfw\flowstations.xyz'&
                    'program aborted in read_flow_ef @ 579'
                stop
              endif
              l=0
...
!             This is written so the order in the nudge flag file can be different from the flow file              
              do i=1,no    ! flowstations.xyz file
                  do l=1,no! flow file
                      ll1 = LEN_TRIM(gname_temp(i))
                      ll2 = LEN_TRIM(flow_sta_name(l))
                      ll1=min(ll1,ll2)
                      if(gname_temp(i)(1:ll1).eq.flow_sta_name(l)(1:ll1))then
...
!c			read(unitNum,*,iostat=ios)(qhyd(l,j),l=1,no)
            if(ios.ne.0)then
                ...
                print*,' problems reading the hydrograph at hour ',j/kt
    ...
    !			BUG: changed nl from mhtot  NK  Nov. 13/03
                    if(qhyd(l,j).gt.0) then
                        if(qhyd(l,j).ge.frc(l,4))then
                            qhyd(l,j)=frc(l,3)+frc(l,1)*(qhyd(l,j)-frc(l,4))**frc(l,2)
                        else
...
!     rev. 10.5.14 June  05/23  = NK Added idskip to stats calculation - set in the evt file
        if(idskip.gt.0)then
            skiplines=nl/kt   ! number in the event files overrides those in the str files
            skipflg=.true.
        else
            skipflg=.true.
            skiplines=0
...
!     rev. 10.4.37 Sep.  10/21  = NK set whyd = -999.0 for stations to be omitted for error calcs
