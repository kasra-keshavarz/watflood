...
d             print*,l,iy(l),jx(l),nn(l)    !ok
              if(nn(l).le.0)then
                  ...
                  print*
                  print*,'WARNING:'
                  print*,'Tracers may not work when flow stations are '
                  print*,'outside the model domain as when using a '
                  print*,'sub-watershed derived from a larger domain '
                  print*,'without making new reach numbers'
                  print*,'Flow station # ',l,' is outside the watershed'
                  print*,'Grid #  ',s(iy(l),jx(l))
                  print*,'inbsnflg',inbsnflg(l)
                  print*,'NOTE:'
...
! trish  had to add the divertflg check
      ...
      if(divertflg.eq.'y')then
        divert: do m=1,nodivert
          n=gridrecieve(m)
          if(n.ge.1.and.n.le.na)then  ! we're in the basin
c          l=1
! NK  Jul. 22/2014         
...
c      if(mod(int(totaltime),kt).eq.0.and.nn(l).gt.0)
      ...
      if(firstpass)then
          do l=1,no
              if(nn(l).le.0)then
                  trace_off=.true.
                  print*,'found flow station outside boundaries'
                  print*,'for flow station # (L) ',l
                  print*,'nn(L) = ',nn(l)
                  stop 'Program aborted in tracer4 @ 546'
              endif
          end do
      endif
...
!        accumulate base flow
         do l=1,no  
            isoconcGWsum(l)=isoconcGWsum(l)
     *                      +qo2(nn(l))*isoconcGW(nn(l),l)
            qo2Sum(l)=qo2sum(l)+qo2(nn(l))
         end do
