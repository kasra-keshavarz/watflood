...
c      implicit none
     ...
     *    SdevAct,SdevSim,KGE 
	integer anallines,nash_count,unitNum,flnNum
...
!     Print to File
      do ii=1,nstations
c      do ii=1,no
	  if(numValues(ii).gt.0.and.SimAvg(ii).gt.0.001.and.nopt(ii).gt.0)then
     ...
     *      ,R2(ii)*abs(Dv(ii))/100.0,numValues(ii),area(ii)
...
!     rev. 10.2.25 May   27/18  - NK: Fixed nash e calculation for value1=nopt=0
	  if(numValues(ii).gt.0.and.SimAvg(ii).gt.0.001.and.nopt(ii).eq.1)then
	    ...
	    dv_mean=dv_mean+abs(dv(ii))
          optim=optim+(1.0-KGE(ii))  ! maximize KGE -> 1.0   so minimize optim
!         check for NaN and write a par file
          if(KGE(ii).ne.KGE(ii))then
	        write(*,*)
              write(*,*)'Error: NaN for KGE'
              write(*,*)'See debug\NaN.par for offending par file'
	        write(unitnum,*)
              write(unitnum,*)'Error: NaN for KGE'
              write(unitnum,*)
     *          'See debug\NaN_par.csv for offending par file'
              filename(99)='debug\NaN_par.csv'
              if(iopt99)call write_par_10(99,99)
              optim=1.0E+30
          else
...
!     rev. 10.4.55 Mar.  29/22  = NK Write KGE to KGE.txt in the working dir for Ostrich
      open(unit=909,file='KGE.txt',status='unknown',iostat=ios)
        if(ios.ne.0)then    ! added Nov. 10/14  nk
          print*
          print*,'Unable to open file  KGE.txt'
        ...
        write(909,*)KGE_mean   ! = KGE
!        write(99,*)'function:,KGE'
!        write(99,*)'Obj_fn:,  ',dds_error
        close(unit=909,status='keep')
...
6003  format(' locn  Nash   KGE       r^2     rms   rms/qbar  %Dv'
     ...
     +     '    area')
6004  format(i4,11f8.2,i10,f12.0)
