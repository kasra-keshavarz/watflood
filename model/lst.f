...
!     SAVES THE LOCAL VARIABLES FROM ONE RUN TO NEXT
      ...
      character(80) :: line
      character(1)  :: dsflg,answer
      logical       :: firstpass,exists,tmpflg,errflg1
...
!     CHARM will look for the file and if present, add the data is so directed    
      ...
      integer       :: day_last,jd,jul_day1_id1,jd1,jd_last
      integer       :: gyear,gmonth,gday,julian
...
!     RAIN,RUNOFF,ETC...
	...
	write(51,6667)scalesnw
      ...
      if(debug_output)then     ! true for debug mode & not compiled  2022-04-15 nk
...
!      write(99,6015)hrs,mins,secs,day,month,year
      ...
      write(51,1013)
      if(writeFlg(1))write(27,1013)(nclass(ii),ii=1,classcount)
...
c            if(aaa.gt.0.0)then
            if(aaa.gt.1.0)then
               ...
               if(statnerr(l).gt.999.)statnerr(l)=999.0
               if(statnerr(l).lt.-999.)statnerr(l)=-999.0
            else
               ab=-1.0
               statnerr(l)=-999.9
            endif
...
!     rev. 9.5.65  Sep.  23/09  - NK: change class frac to whole basin values
              ...
              if(nopt(l).ne.1)statnerr(l)=-999.
                
!             write only where there is data
	        i=(ystr(l)-yorigin)/ydelta+1
	        j=(xstr(l)-xorigin)/xdelta+1
!              print*,l,aaa,inbsnflg(l),nopt(l)
              if(aaa.gt.ppsum(l)*0.01.and.
     *             inbsnflg(l).eq.1.and.
     *             nopt(l).gt.0)then
...
c      name='WATFLOOD model output: recorded/computed'
      name='WATFLOOD model output: computed'
...
!     rev. 10.4.53 Mar.  14/22  = NK Modifications for weather model diversio inflowg
      if(writeFlg(2))then
        write(line,95400)id
95400   format('results\',i3.3,'_str.tb0')
        read(line,*)fln(955)
        print*,fln(955)(1:40)
       
        call write_tb0(955,955,1,no,8)
     ...
     *         'Error with allocation of outarray lst & 511'
        ...
        do k=kt,nl,kt
          do l=1,no
            outarray(k/kt,l)=qsyn(l,k)
c            outarray(k/kt,l*2-1)=qhyd(l,k)
c            outarray(k/kt,l*2)=qsyn(l,k)
          end do
        end do
...
!       write the data to spl.tb0
        noDataValue=-999.0
c        if(writeFlg(1))call write_tb0(955,955,0,no*2,12)  
!       *******************************************************        
        call write_tb0(955,955,0,no,12)    
!       *******************************************************        
        close(unit=955,status='keep')
...
!     Write or modify a diversion file  
!     Used to transfer computed flows from one run to the nexr
!     e.g. from upstream basin to a downstream one 
!     input file name = fln(52)
!     Create a div file in the diver ````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````dir of the receiving basin
!     e.g. Trent into the Crowe (-Otanabee)      
!     first read the locations of the receiving grids for downstream basin:
!     write the diversion file      
      if(wrtdiverflg)then
!         create the output file names
          strLength=LEN_TRIM(outputDirectory)
!          if(day_now.eq.1)then
!              write(fln(99),10200)outputDirectory,year_now-1
!          else
              write(fln(99),10200)outputDirectory,year_now
!          endif
10200     format(a<strLength>,i4,'0101_div.tb0')    
!         This overrides the above:  
          if(ni.gt.1)then   ! event_fln is not dimensioned to ni = 1
              if(event_fln(id)(1:10).eq.'event\regl')then
                  write(fln(99),10110)outputDirectory
10110             format(a<strLength>,'regl_div.tb0')              
              elseif(event_fln(id)(1:9).eq.'event\glb')then
                  write(fln(99),10160)outputDirectory
10160             format(a<strLength>,'glb_div.tb0')              
              elseif(event_fln(id)(1:13).eq.'event\climate')then
                  write(fln(99),10170)outputDirectory
10170             format(a<strLength>,'climate_div.tb0')  
              elseif(event_fln(id)(1:15).eq.'event\ens_long_')then
                  write(fln(99),10180)outputDirectory
10180             format(a<strLength>,'ens_long_div.tb0')  
              elseif(event_fln(id)(1:10).eq.'event\ens_')then
                  write(fln(99),10190)outputDirectory
10190             format(a<strLength>,'ens_div.tb0')  
              endif
          endif
          ...
          do l=1,nDivOut
            gname(l)=divOutName(l)
          end do
          do l=1,nDivOut
            column_units(l)='cms' 
            column_type(l)='float'
            xsta(l)=divIn_xsta(l)    ! take
            ysta(l)=divIn_ysta(l)    ! take
            xsta1(l)=divOut_xsta(l)    ! give
            ysta1(l)=divOut_ysta(l)    ! give
          end do
          noDataValue=-999.0   
          
!         write the header for each event
          call write_tb0(99,99,1,nDivOut,8)
     ...
     *              'problem deallocating outarray in lst @ 573'
          ...
          allocate(outarray(nl,nDivOut),stat=iAllocate)
          if(iAllocate.ne.0)STOP
     *             'Error with allocation of outarray lst & 1062'
              
          do l=1,nDivOut
              do k=1,mhtot
                  outarray(k,l)=qdiv(l,k)
              end do
          end do
...
!         write the data to spl.tb0
          noDataValue=-999.0
          call write_tb0(99,99,0,nDivOut,12)    
...
!         close each event.      
 ...
 !    END rev. 10.4.32 Jan.  10/21  = NK Added diversion output
...
c        if(dds_flag.ne.1.and.iopt.lt.99)then
        if(iopt99)then   ! changed 2022-04-15  NK
        author='WATFLOOD/CHARM                          '
...
!            outarray(i,j)=amax1(basinerr(i,j),-999.000)
            outarray(i,j)=amin1(basinerr(i,j),50.000)
            outarray(i,j)=amax1(basinerr(i,j),-50.000)
	    end do
	  end do
...
!     rev. 10.4.59 aUG.  30/22  = NK Added class ET to basin_no.xyz for iopt>=1 
        ...
        write(99,99998)(ii,ii=1,classcount)
99998   format('      long       lat      bsn#',<classcount>i10)     
        do i=1,ycount
          do j=1,xcount
            if(s(i,j).gt.0)
     *          write(99,99999)float(j)*xdelta+xorigin-0.5*xdelta,
     *               float(i)*ydelta+yorigin-0.5*ydelta,
     *               nhyd(i,j),(sum_et(s(i,j),ii),ii=1,classcount)      
99999       format(2f10.5,i10,<classcount>f10.1)            
          end do
        end do
        close(unit=99,status='keep')
        endif  
...
60001         format(a10,<no>(',',1x,a7,a4,',',1x,a7,a4))     
      endif
...
!     For WATROUTE  output is wrt.csv and NIT spl.csv
        ...
        if(nudgeflg.eq.'y'.and.flowresetflg.eq.'y')then
          do l=1,no
            do k=kt,nl,kt
c             qsyn(l,k)=-999
            end do
...
!      Write results\WF-HRDPS-7day-forecast.csv for GSCA
!      print*,iyear,imonth,iday
!      print*,year_now,month_now,day_now
!     i*** is the computer calendar yyyy/mm/dd        
      if(fln(1)(1:17).eq.'basin\SGB_shd.r2c'.and.year_now.eq.iyear)then
        if(id.eq.1)then
          write(20,20002)iyear,imonth,iday  
20002     format(i4.4,'/'i2.2,'/',i2.2,
     *        '-5 days 01:00 UTC start + 48hr HRDPS + 120hr GDPS ')          
          write(20,20001)(gage(l),l=1,no) 
20001     format(4x,<no>a12)
          do k=nl-120,nl
            write(20,20000)(qsyn(l,k),l=1,no)
c            write(*,20000)totaltime,(qsyn(l,k),l=1,no)
20000       format(<no+1>(f10.3,', '))           
          end do
        elseif(id.eq.2)then
          do k=1,48
            write(20,20000)(qsyn(l,k),l=1,no)
...
!     rev. 10.2.15 Feb.  05/18  - NK: Added 'results\monthly_peaks'
      if(iopt99)then
        do j=1,mhtot/732+1
!     rev. 10.2.41 Dec.  10/18  - NK: Added winter monthly peaks
          if(yorigin.ge.47.0)then
            if(j.le.6.or.j.eq.12)then
...
!     calculate and write the daily flows to ..\simout\charm_dly.csv
!     turned off for FEWS AND iopt=0
c      if(kt.lt.24)then    won't work for other dt's or more than 1 year
      if(kt.eq.1.and.writeflg(71))then
        if(id.eq.1)then 
!        write the headers for charm_dly.csv
              jd1=julian(year_start,mo_start,day_start)-1
              write(71,60003)
     *            (gage(l),'_obs',gage(l),'_SIM',l=1,no)
60003         format('   date   ',<no>(',',1x,a7,a4,',',1x,a7,a4))     
        endif
        ...
        if(writeflg(71))then   ! charm_dly.csv
          if(id.eq.1)jd=jd1  
          do j=1,nl/24,kt
              jd=jd+1
              call GREGORIAN (JD,gYEAR,gMONTH,gDAY)
              write(71,7404)gyear,gmonth,gday,
     *                (qhyd_dly(l,j),qsyn_dly(l,j),l=1,no)
              jd_last=jd
          end do
          if(jd_last-jd1.le.364.and.id.eq.ni)then
c              jd1=jd_last
            do j=jd_last-jd1+1,365
              jd=jd+1
              call GREGORIAN (JD,gYEAR,gMONTH,gDAY)
              write(71,7404)gyear,gmonth,gday,
     *                (-999.0,-999.0,l=1,no)
...
!             run through the months
              do k=1,mohours(j)
!     rev. 10.4.40 Sep.  28/21  = NK Fixed bug in lst: # hours in partial months
                if(hour_no.eq.mhtot)exit
                hour_no=hour_no+1    ! count hours
...
c        if(.not.contflg.eq.'y'.and.iopt99)then
        if(.not.contflg.eq.'y'.and.writeflg(80).or.iopt99)then
...
c      if(.not.netCDFflg.and.dds_flag.ne.1.or.iopt99)then           
      if(writeflg(80).and.dds_flag.ne.1.or.iopt99)then           
...
!          if(id.eq.ni-1.and.mhtot.le.2400)then   ! regl forecast
          if(mhtot.eq.48)then   ! regl forecast
              i=0
              do k=deltat_report,mhtot,deltat_report
                  i=i+1
                  do l=1,noresv
...
c          elseif(id.eq.ni.and.mhtot.le.2400)then  ! glb forecast
          if(mhtot.eq.192)then  ! glb forecast
              i=2
              do k=deltat_report,mhtot,deltat_report
                  i=i+1
                  do l=1,noresv
...
c 7404 format(i8,<no*2>(',',g12.4))
 7404 format(i4.4,'-',i2.2,'-',i2.2,<no*2>(',',g12.4))
...
c6671 format('  l,next,subarea(l),suberr(l)/',2i5,4f9.1)
 ...
 6679 format(a60)
 7601 format(i8,<2*(no+noresvi)>f8.1)
 7602 format(80E11.3)
! 7777 format(10f10.1)
