...
!     rev. 9.1.55  Jun.  12/04  - NK: write new str files to strfw\newfmt folder.
	...
	character(14) :: date
      ...
      real*4, dimension(:),   allocatable :: Qmaxrel
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~START FIRST PASS~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     *             Qmaxrel(noresv),qqold(noresv,9999),stat=iAllocate)
...
c        do l=1,noresv
c            if(resname(l)(1:6).eq.'AhmicL')then
c                print*,'Max flow for AhmicL - 0 for no limit:'
c                read*,Qmaxrel(l)
c                if(qmaxrel(l).le.0.00001)qmaxrel(l)=99999.0
c            elseif(resname(l)(1:7).eq.'CecebeL')then
c                print*,'Max flow for CecebeL - 0 for no limit:'
c                read*,Qmaxrel(l)
c                if(qmaxrel(l).le.0.00001)qmaxrel(l)=99999.0
c            else
c                qmaxrel(l)=99999.0
...
!           this reservoir has a rule          
            ...
            resvNo(ll)=res(rank)
            write(53,53000)ll,rank,res(rank),xrule(ll*2),yrule(ll*2)
53000       format('Rule #',i5,' grid # ',i5,' res # ',i5,2f10.3)            
          else  
            warningflg=.true.
            print*,'Rule coordinates do not match a reservoir location'
            print*,'Or we`re outside the watershed'
      ...
      print*  
        ...
        if(warningflg)then
          ...
          PRINT*,'ok if using a sub-watershed'
...
c        do j=1,365
        do j=1,8737,24
          read(99,99,iostat=ios)(chr(i),i=1,1024)
99        format(1024a1)
          do i=1,1024
              if(chr(i).eq.'/')chr(i)='-'
          end do
          write(line,99)(chr(i),i=1,1024)
          read(line,*,iostat=ios)yyyymmddhh(j),hhmmss(j),
     *                    (lower_range(j,i),upper_range(j,i),i=1,nrules)
          if(ios.ne.0)then
              print*,'problen reading the data in the rules.ts5 file'
              print*,'at ',j,yyyymmddhh(j)
              write(line,99)(chr(i),i=1,1024)
              stop 'Program aborted in rules_sl @ 222'
          endif
!          print*,j,yyyymmddhh(j),lower_range(j,1),upper_range(j,1)
        end do
...
!         storage = live storage
     ...
     *              +lower_range(j,ruleNo(l)))/2.0
          ...
          if(decayT(l).gt.100)then
            ...
            qo2(n)=qo1(n)
            if(l.eq.19)then
c                write(333,33300)time,qo2(n),(qqold(l,j),
c     *                                       j=1,decayT(l))
c33300           format(999f10.3)   
...
!     rev. 10.4.54 Mar.  14/22  = NK Shut off power palnt when Muskoka lake = low
        if(resname(1)(1:7).eq.'Muskoka')then
            if(jul_day_now.gt.85)then   ! Mar. 26
                if(lake_elv(l,jz).lt.lower_range(j,ruleNo(l)))then
                    qdivert(i,jz)=0.0
                endif
            endif
        endif
        
c        if(l.eq.19)write(333,33300)
c        if(l.eq.19)write(333,33300)time,qo2(n)
...
c        qtemp=min(qmaxrel(l),qtemp)
        
        
c        if(l.eq.19)write(333,33300)time,qtemp
...
!       if(l.eq.1)write(333,*)time,qo2(n),
!     *              (qqold(l,j),j=1,3)
           
       
!     rev. 10.4.56 Apr.  24/22  = NK Added qmin() to rules_st.f
        qo2(n)=amax1(qo2(n),qmin(l))
