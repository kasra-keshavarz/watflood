...
!     SAVES THE LOCAL VARIABLES FROM ONE RUN TO NEXT
      ...
      INTEGER           :: nratio,nj,attCount,iallcnt1,m,i,j,ios,n,ii,
     *                     iallocate
      logical           :: exists
...
!     rev. 10.4.33 May.  26/21  = NK Added totUZS, API & heatdef to the wfo file
        ...
        read(99,99004,iostat=ios)attCount    ! attribute count
        if(ios.ne.0.or.wfo_spec_version_number.ne.7.0)then    ! see below <<<<<<!!!!
          print*,' Outdated wfo_spec.txt'
          print*,' You may need to update the wfo_spec.txt file'
          print*,' File version number =',wfo_spec_version_number
          print*,' Required version number is > 6.0'
          print*,' Latest version = 7.0'
          print*,' Run the current bsn64*.exe to create a new'
          print*,' basin\wfo_spec.new file and copy it to the'
          print*,' working directory as wfo_spec.txt'
          print*
!     rev. 10.4.33 May.  26/21  = NK Added totUZS, API & heatdef to the wfo file
          if(wfo_spec_version_number.eq.6.0)then 
              nj=15+15*(classcount)  
              print*,'found version 6.0'
              print*,'Charm continues with this'
          else
              stop ' Program aborted in rd_wfo_spec @ 1195'
          endif
        else
            nj=17+16*(classcount)   !
        endif
...
! Note:  number below needs to be updated if items are added <<<<<<<<<<<<<<<<<!!!!!
        ...
        if(attCount.ne.nj)then
          ...
          print*,' No entrees found =',attCount
          if(wfo_spec_version_number.eq.7.0)then
              print*,' Looking for', 17+16*(classcount)
          elseif(wfo_spec_version_number.eq.6.0)then
