...
!  ydum - converts mm of water depth to cms for the time step
!         so depth in mm is multiplied by ydum, flow is divided by ydum
...
!     SAVES THE LOCAL VARIABLES FROM ONE RUN TO NEXT
      ...
      real*4   :: power1,thr,xdum,t,ydum,e1,qlzfrac,a51,zj,time,
...
c      data firstpass/.true./
      if(debug_output)print*,' checkpoint 0a in runoff'
...
!     XDUM CONVERTS 1 mm depth to cu. m. for the centre grid
      xdum=1000.*step2
!     ydum converts 1 mm/hour depth to cms for the centre grid
      ydum=1000.*step2/t
...
!       rev. 9.8.61  May   22/13  - NK: Introduced flag1 to speed up runof6
      ...
      if(debug_output)print*,' checkpoint 0b in runoff'
...
!     rev. 10.5.12 May   26/23  = NK Fixed strloss & qstream
!        grid_area(n)/3600000.0*frac(n)*aclass(n,ii_water
        allocate(tdum(na),stat=iAll)
	    if(iAll.ne.0) STOP 'Error allocating arrays in aet.f90'  
        do n=1,naa
!          tdum converts mm/hr for cms on channel surface area            
           tdum(n)= 1000.*step2/3600.*frac(n)*aclass(n,classcount-1)
        end do
...
!     * * * * INITIAL VALUES * * * *
!
      if(debug_output)print*,' checkpoint 1 in runoff. JAN=',jan
...
!     rev. 10.2.39 Nov.  15/18  - NK: changed snowc(n,ii) to snowc(n,ii)*sca(n,ii) in runof6
     ...
     *         tto(n),ttomin(n),dd_thaw(n),dd_ice(n),ice_fctr(n)
             end do
        endif
      endif   
      ...
      if(debug_output)print*,' checkpoint 1b in runoff- after WATBAL'

1002  CONTINUE                         ! PRODUCTION MODE

cd      if(iopt.eq.3)print*,' checkpoint 2 in runoff'

      a51=a5**thr    ! thr = t/3600  t = # hours to next precip
      zj=p(ipr,jpr)
...
c????????????????????????????????????????????????????????????
      ...
      if(debug_output)print*,' checkpoint 2a in runoff'
...
cd         if(iopt.eq.2)    print*,' back from aet'
      ...
      if(debug_output)print*,' checkpoint 2b in runoff'
...
!     THE S/R IS CALLED AT LEASR ONCE TO SEE IF THERE IS SNOW
      ...
      if(debug_output)print*,' checkpoint 2c in runoff'
...
cc!$OMP PARALLEL DO   NUM_THREADS(2)
      ...
      do n=1,naa
        ...
        if(debug_output)print*,'In runof6, passed 101'
      ...
      if(debug_output)print*,'In runof5, passed 201'

        qlz(n)=0.0
        
        do ii=1,classcount   ! changed Mar. 31/06 nk

      if(debug_output)print*,'In runof5, passed 301'

!         these variables added Jul. 7/06  nk
          eff_bare_area=frac(n)*aclass(n,ii)*(1.0-sca(n,ii))
          eff_sc_area=frac(n)*aclass(n,ii)*sca(n,ii)

!     rev. 9.5.15  Feb.  28/08  - NK: fixed ydum & xdum for proper grid area in lat-long
...
!     *       PAUSE 'channel runof5 loop for wetland class'
      ...
      if(debug_output)print*,'In runof5, passed 401'
...
!   ++++++++++             BARE GROUND                ++++++++++
      ...
      if(debug_output)print*,'In runof5, passed 501'

!     rev. 10.5.12 May   26/23  = NK Fixed strloss & qstream
!            if(ak(ii).gt.0.0)then
            if(ii.eq.ii_water)then
!             WATER  WATER  WATER  WATER  WATER  WATER  WATER  WATER                 
!             WATER  WATER  WATER  WATER  WATER  WATER  WATER  WATER                 
!             WATER  WATER  WATER  WATER  WATER  WATER  WATER  WATER                 
!             for the water class only
              if(ireach(n).eq.0)then
!               tdum converts mm/hr for cms on channel surface area            
!                qstream(n) is added to channel inflow in route
!                stream(n) = net melt+rain in mm converted to cms
!                qstream(n)=qstream(n)+fexcess(n,ii)*eff_sc_area*tdum(n)

                qstream(n)=r(n,ii_water)*tdum(n)            ! *(1.0-sca(n,ii_water))
                strloss(n)=ev(n,ii_water)*fpet(ii_water)*tdum(n)  ! same as in lake_evap
...
!               AND ALL THE WATER IS INFILTRATED-NO RUNOFF
                ...
                if(frcflg.eq.'y')then
                  qdf(n,ii)=qdf(n,ii)+d1(n,ii)*eff_bare_area*ydum
                endif
...
!               prorated for sca Jul. 7/06 nk
                ...
                if(frcflg.eq.'y')then
                  qdf(n,ii)=qdf(n,ii)+fake(ii)*eff_bare_area*ydum
                endif
              endif

      if(debug_output)print*,'In runof5, passed 801'
...
!                 duz=1.0e-10
      ...
      if(debug_output)print*,'In runof5, passed 1001-2'
...
!     rev. 10.1.84 May   09/17  - NK: Put drng(n,ii)=drng(n,ii)*fraction back into runof6
                  ...
                  qint(n,ii)=duz*eff_bare_area*ydum
                  if(n.eq.nnprint)then
                    sq1(ii)=sq1(ii)+rf(n,ii)*eff_bare_area*ydum
                    sqint(ii)=sqint(ii)+duz*eff_bare_area*ydum
                  endif
                  if(n.eq.nnprint)then
                    sdrng(ii)=sdrng(ii)+drng(n,ii)*eff_bare_area  
!                   TS: added *ydum b/c lower zone inflow: Nov 1/07
                    slzinflw=slzinflw  ! FLOW
     *                     +drng(n,ii)*eff_bare_area*ydum 
                  endif
...
!                 LAND COVER AREA OF EACH CLASS        ! lzs is depth
      ...
      if(debug_output)print*,'In runof5, passed 1301'
...
!               to see what the recharge is on each grid.
                ...
                qdrng(n)=qdrng(n)+drng(n,ii)*eff_bare_area*ydum
                if(frcflg.eq.'y') qdrng2(n,ii)=
     *                       qdrng2(n,ii)+drng(n,ii)*eff_bare_area*ydum
...
!               INTERFLOW
      ...
      if(debug_output)print*,'In runof5, passed 1401'
...
!           EROSION ON BARE GROUND ONLY
      ...
      if(debug_output)print*,'In runof5, passed 1601'
...
c                sumffs(n,ii)=sumffs(n,ii)+d1fs(n,ii)
                ...
                if(frcflg.eq.'y')then
                  qdffs(n,ii)=qdffs(n,ii)+d1fs(n,ii)*eff_sc_area*ydum  ! fixed nk
                endif
!                prorated for sca Jul. 7/06 nk
                d1fs(n,ii)=0.0
              else
...
!                prorated for sca Jul. 7/06 nk
                ...
                if(frcflg.eq.'y')then
                  qdffs(n,ii)=qdffs(n,ii)+fakefs(ii)*eff_sc_area*ydum
                endif
              endif

      if(debug_output)print*,'In runof5, passed 1801'
...
!     rev. 10.1.84 May   09/17  - NK: Put drng(n,ii)=drng(n,ii)*fraction back into runof6
                  ...
                  qintfs(n,ii)=duz*eff_sc_area*ydum
                  if(n.eq.nnprint)then
                    sq1fs(ii)=sq1fs(ii)+rffs(n,ii)*eff_sc_area*ydum
                    sqintfs(ii)=sqintfs(ii)+duz*eff_sc_area*ydum
                  endif
                  if(n.eq.nnprint)then
                    sdrngfs(ii)=sdrngfs(ii)+drngfs(n,ii)*eff_sc_area
                    slzinflw=slzinflw  !FLOW
     *                      +drngfs(n,ii)*eff_sc_area*ydum
                  endif
...
!             TS: MADE THIS DRNG FROM SNOWMELT FOR TRACER S/R (08/27/03)
              qdrngfs(n)=qdrngfs(n)+drngfs(n,ii)*eff_sc_area*ydum
              if(frcflg.eq.'y') qdrngfs2(n,ii)=
     *                     qdrngfs2(n,ii)+drngfs(n,ii)*eff_sc_area*ydum
!             STREAMFLOW CONTRIBUTION: OVERLAND & INTERFLOW
!             WEIGHTED ACCORDING TO LAND COVER AMOUNT
...
!             INTERFLOW
      ...
      if(debug_output)print*,'In runof5, passed 2201'
...
c              qstream(n)=qstream(n)+fexcess(n,ii)*eff_sc_area*ydum
c              
c              strloss(n)=0.0
c              
...
!           END OF SNOW COVERED AREA COMPUTATIONS
      ...
      if(debug_output)print*,'In runof5, passed 2301'
!  ++++++++++++++++++++++++++++++++++++++++++++++++++++++   fs end 
...
!        endif
      if(debug_output)print*,' checkpoint 5 in runoff'
!     rev. 9.4.08  May.  29/07  - NK: changed baseflow argument list
!       ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        call baseflow(n,dlz,sdlz,ydum)
!       ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      if(debug_output)print*,' checkpoint 6 in runoff'
        if(frcflg.eq.'y')then
          storeGW2(n)=lzs(n)*xdum*frac(n)
        endif
...
!     rev. 9.2.42  Jun.  20/06  - NK: water class included in the water balance
        sumrff(n)=sumrff(n)+qr(n)/ydum/frac(n)
...
cccccccccccccccccccccccccc        qstream(n)=0.0                   
      if(debug_output)print*,' checkpoint 8 in runoff'
...
!     rev. 9.4.06  May.  09/07  - NK: replaced por with spore(n,ii) in runof6
      ...
      if(debug_output)print*,n,ii,time,' checkpoint 9 in runoff'
        end do                !  n=1,naa loop
...
c 6000 format(' ',f8.0,999(',',g10.3))
 ...
 6000 format(' ',f8.0,3(',',g10.3),',',f10.1,53(',',g10.3))
