...
!     rev. 10.4.35 Aug.  31/21  = NK Added check on ws(n) for NAN in etharg_beta
          if(ws(n).ne.ws(n))ws(n)=0.0   ! check for NAN
          so(n)=15.392*dr*
     *           (ws(n)*sinlat(n)*sindlta+coslat(n)*cosdlta*sin(ws(n)))
        end do
c        print*,ju
      endif
c      write(765,*)totaltime,so(nnprint),dly_diff(nnprint),pet(nnprint,2)
...
!        if(n.eq.nnprint)print*,ju,ws(n),so(n),petn,tempv(n)
        do ii=1,classcount
          pet(n,ii)=petn
        end do
      end do
