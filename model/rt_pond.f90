...
!     rev. 10.2.44 Jan.  21/19  - NK: Added qOld() to allow previous weighted outflow on the resume file
        hold=0.1e+26
        ic=0
!c         using a power function          - doubled ic 2023-05-31
          if(store2(n).gt.0.0)then
            do while(abs(hold-store2(n)).gt.0.003*hold.and.ic.lt.40)
                ic=ic+1
                if(store2(n).gt.0.0)then
...
!c                  qo2(n)=qo2(n)*lake_ice_factor(l,month_now)
                  NaNtest=ISNAN(qo2(n))
                  if(NaNtest)then
                      write(98,*)'Error: pond routing NaN in grid ',n,'at t= ',totaltime
                      write(98,*)'Error: Lower coefficient to avoid error'
                      write(98,*)'Error: done automaticaly to keep program running'
                      write(*,*)'Error: pond routing  NaN in grid ',n,'at t= ',totaltime
                      write(*,*)'Error: Lower coefficient to avoid error'
                      write(*,*)'Error: done automaticaly to keep program running'
                      rlake(n)=rlake(n)*0.10
                      write(98,*)'Warning: rlake(n) set to : ',rlake(n)
                      write(98,*)'Warning: for basin :',ibn(n)
                      write(*,*)'Warning: rlake(n) set to : ',rlake(n)
                      write(*,*)'Warning: for basin :',ibn(n)
                      error_msg=3
                  endif
                  wt=amax1(0.1,float(ic)/41.0)
...
!c                  qold(n)=qo2(n)
            ...
            if(ic.eq.20)then
                print*,'no convergence in n= ',n,' time= ',totaltime
                pause 44444
            endif
