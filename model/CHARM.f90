...
!     rev. 9.8.85  Sep.  30/13  - NK: Fixed the water balance for Lake St. Jo so 9sion is taken care of
...
!     rev. 10.4.28 Oct.  14/20  = NK Wetland-Pond routing
!     rev. 10.4.29 Nov.  23/20  = NK netCDF - nc output for r2c & tb0 inputs
!     rev. 10.4.29 Nov.  26/20  = NK Code review - add missing  "implicit none" & "save"
!     rev. 10.4.30 Dec.  18/20  = NK Added a13 parameter for rainsnowtemp > base tmp
...
!     rev. 10.4.35 Aug.  31/21  = NK Added check on ws(n) for NAN in etharg_beta
!     rev. 10.4.36 Sep.  05/21  = NK Glacier tmp kludge for cmc regl 2m tmp
!     rev. 10.4.37 Sep.  10/21  = NK set qhyd = -999.0 for stations to be omitted for error calcs
!     rev. 10.4.38 Sep.  26/21  = NK no nudging for flow with ice cover
!     rev. 10.4.39 Sep.  26/21  = NK Set nopt for hdrflg = '0' only
...
!     rev. 10.4.45 Nov.  06/21  = NK Fixed starting data for wfo file in wfocode
!     rev. 10.4.46 Dec.  30/21  = NK Fixed starting data for wfo file in wfocode for ID=1
...
!     rev. 10.4.51 Feb.  07/22  = NK Modified DD_thaw - moved reset date earlier
!     rev. 10.4.52 Feb.  25/22  = NK Added swe.txt for swe on debug grid for plotting
!     rev. 10.4.53 Mar.  14/22  = NK Modifications for weather model diversion inflow
!     rev. 10.4.54 Mar.  14/22  = NK Shut off power palnt when Muskoka lake = low
!     rev. 10.4.55 Mar.  29/22  = NK Write KGE to KGE.txt in the working dir for Ostrich
!     rev. 10.4.56 Apr.  24/22  = NK Added qmin() to rules_st.f
!     rev. 10.4.57 Jan.  29/22  = NK added smrflg = "d" for Disaggredation on CHARM for Ostrich     
!     rev. 10.4.58 Jun.  21/22  = NK fixed nudgeflg to be off when ='n'
...
!     rev. 10.5.02 Jan.  13/23  = NK Set nopt default to 1 for flow.nc file for FEWS
!     rev. 10.5.03 Feb.  08/23  = NK Fixed swe & sca upon resume - was entered 1 day late
...
!     rev. 10.5.07 Apr.  25/23  = NK Fixed diversion to prevent outflow > available storage
...
!     rev. 10.5.10 May   11/23  = NK Put restriction on evaporation in lake evap
!     rev. 10.5.11 May   17/23  = NK Fixed diversion for source outside the domain @ n=na
!     rev. 10.5.12 May   26/23  = NK Changed strloss & qstream for NCA
!     rev. 10.5.13 May   26/23  = NK Fixed diversion outlet check:  naa -> na
!     rev. 10.5.14 June  05/23  = NK Added idskip to stats calculation - set in the evt file
...
!     rev. 10.5.18 Aug.  13/23  = NK Updated Hype code for new parameter entries
!     rev. 10.5.19 Aug.  26/23  = NK Fixed tracer output for location outside domain
      program_name='CHARM     '
      program_version=' 10.5.19  '
      program_date='2023-08-26'
...
! unit=252  fln(22)- spare                          yyyymmdd_***.***
...
! unit=257  fln(27)- SPARE                          yyyymmdd_***.***
...
!           fln(63)- Also used in writing netCDF error debug\*_netCDF_errors.txt
...
!      conv=sqrt(0.0/0.0)
!      print*,conv
!      if(conv.ne.conv)then
!          print*,'true - it`s not equal'
!          stop 1
!      else
!          stop 2
!      endif
...
!     units 31-50 reserved for input files
      ...
      filename(55)='debug\route_init.txt'  !routing data used to be rte.txt
      filename(56)='debug\rte.csv'      ! routing debug info & ice stuff
...
!     for WATROUTE this file is named results\wrt.csv      
      ...
      filename(64)='results\swe.txt'      !swe on the debug grid nnprint
...
!     rev. 9.9.24  Aug.  20/14  - NK: Added monthly mean flow csv file CHARM_mly_nn.csv
      ...
      open(unit=20,file=filename(20),status='unknown',iostat=ios)
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!        if(ios.ne.0)call io_warning(20,filename(20),ios)
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      if(ios.ne.0)then
          stop 'CHARM aborted - read only error debug\warnings.txt'
          call io_warning(98,filename(98),ios)
      endif
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!     rev. 10.2.38 Oct.  24/18  - NK: Added runReport.txt for forecast mode 
      ...
      if(exists)then
        ...
        if(ni.ge.2)then
!           Note: event_fln not allocated for ni = 1 - i.e. no extra events            
            write(99,*)'CHARM.exe - started            ',&
             time(1:2),':',time(3:4),':',time(5:6),' ',event_fln(ni)(6:30)
        else
            write(99,*)'CHARM.exe - started            ',&
...
!       but strloss taken out!!!!!!
        ...
        dd_thaw(na),dd_ice(na),dd_ice_min(na),dd_ice_sum(na),stat=iAllocate)
...
!       **********************************************************************
        if(dds_flag.ne.0)call write_par_10(99,26)
!       **********************************************************************
...
!       **********************************************************************
        ...
        iopt99=.false.    ! used in liu of iopt
...
!         and should get an entry in the event file
          ...
          if(exists)then
           write(98,*)'Info: found basin\monthly_climate_normals.txt file'
             ...
             write(*,*)'Error: Could not find:"basin\monthly_climate_normals.txt"'
...
!   NAMES WILL BE USED.
      ...
      if(dds_flag.eq.1)then
          iopt=0
          iopt99=.false.   ! suppres all unneeded fiels for dds
      endif
...
!           suppress output for netCDF production runs when iopt = 0          
            ...
            writeflg(2)=.false.      ! used as a generic flag that can be used anywhere
            ...
            writeflg(71)=.false.     ! charm_dly.csv
            writeflg(78)=.false.
            writeflg(80)=.false.      !lake_SD.csv
...
!     time & grid loop - so will save a lot of time
      ...
      if(debug_output)write(51,*)'Wetland flag settings:'
      do n=1,naa
        if(wetflg.eq.'y'&
                .and.grid_area(n).gt.10.0&   
                .and.aclass(n,classcount-2).gt.0.0&         
!     rev. 10.4.28 Oct.  14/20  = NK Wetland-Pond routing
                .and.aclass(n,classcount-2).gt.aclass(n,classcount-1)&         
                .and.theta(n).gt.0.00001&        
                .and.glacier_flag(n).ne.'y')then     
          wetland_flag(n)=.true.
!         can not have wetland & pool routoing at the same time
!         wetland takes precidence as it get rid of water
          pool(n)=0.0
        else
          wetland_flag(n)=.false.
        endif
        if(debug_output)write(51,*)n,grid_area(n),aclass(n,classcount-2),ibn(n),theta(n),glacier_flag(n),wetland_flag(n)
    end do  
...
!     rev. 10.1.01 Oct.  05/15  - NK: Isotope update: added 2H
 ...
 1999 FORMAT(<nisoframe>(6(f20.6,','),',',',',',',','))
    ...
    print*
      ...
      if(error_msg.eq.2)then
        write(*,*)'Error:'
        write(*,*)'Error: channel routing NaN  '
        write(*,*)'Error: check debug\warnings.txt for details '
      endif
      if(error_msg.eq.3)then
        write(*,*)'Error: pond routing NaN '
        write(*,*)'Error: check debug\warnings.txt for details '
      endif
...
!     rev. 10.1.98 Oct   04/17  - NK: Deal with -ve flows in route
      ...
      if(warning(3))then
          write(98,*)'Warning:'
          write(98,*)'Warning: Wetland outflow set to 0.0 for that time step'
          write(98,*)'Warning: and CHARM continues'
          write(98,*)'Warning: See `wetland NaN` above'
      endif
      write(98,*)
      write(98,*)'DISCLAIMER'
