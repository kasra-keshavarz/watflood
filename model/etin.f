...
!     rev. 10.4.51 Feb.  07/22  = NK Modified DD_thaw - moved reset date earlier
      do n=1,naa
         ddh=tempv(n)/24.
!      ACUMMULATION OF DEGREE-DAYS
         
!        Ice         
!        This works because the dd_ice reset day can be set anytime before any frost
!        and should be.         
         if(tempv(n).lt.0.0)dd_ice(n)=dd_ice(n)-ddh  !makes it earlier
         
!        This works because the dd_thaw reset day can be set anytime before any + tmp's
!        and should be.         
         if(tempv(n).gt.0.0)dd_thaw(n)=dd_thaw(n)+ddh   !makes it earlier

         tto(n)=tto(n)+ddh     
         ttomin(n)=amin1(ttomin(n),tto(n))
         ttomax(n)=amax1(ttomax(n),tto(n))
