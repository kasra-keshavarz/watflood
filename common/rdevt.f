...
!     OPEN THE EVENT FILE:
      ...
      open(unit=99,file=fln(99),iostat=ios)
        ...
        if(ios.ne.0)then
          write(*,99921)fln(99)
          if(numa.le.0)write(51,99921)fln(99)
...
!         REMEMBER WHICH MONTH WE START IN:
        ...
        if(numa.le.0)write(51,*)junk,day1         
...
!       turn these off for opt
	  ...
	  deltat_report=1  ! default for writing gridflow.r2c
...
!           only ireport from the first event 
!           so skip reading in subsequent events
	      read(99,*,iostat=ios)junk,junk
          endif
	  endif
        read(99,*,iostat=ios)junk               ! #
        if(iopt.eq.2.or.ios.ne.0)print*,junk
...
!         write this stuffto spl.txt for the record
          ...
          write(51,*)':deltat_report                ',deltat_report     
          write(51,*)'#                             '
        endif
...
!     _shd.r2c    
        ...
        if(numa.le.0)write(51,1030)i+30,i,fln(i)
!     .par    
        i=2
        read(99,99005,iostat=ios)junk,fln(i)
	  if(id.eq.1)par_fln=fln(i)
        if(iopt.eq.2.or.ios.ne.0)print*,junk,i,'   ',fln(i)
        if(numa.le.0)write(51,1030)i+30,i,fln(i)
!     ch_par.r2c  
!     rev. 9.3.11  Feb.  28/07  - NK: ch_par added / event file ver = 9.5
	    ...
	    i=41
          read(99,99005,iostat=ios)junk,fln(i)
          if(iopt.eq.2.or.ios.ne.0)print*,junk,i,'   ',fln(i)
          if(numa.le.0)write(51,1030)i+230,i,fln(i)
c          write( *,1030)i+230,i,fln(i)
	  endif
!     .pdl   
        i=3
        read(99,99005,iostat=ios)junk,fln(i)
...
!    .wqd 
        if(evt_version.ge.9.3)then
          ...
          read(99,99005,iostat=ios)junk,fln(i)
          if(iopt.eq.2.or.ios.ne.0)print*,junk,i,'   ',fln(i)
          if(numa.le.0)write(51,1030)i+250,i,fln(i)
        endif
...
c     add snow,drain,dsnow  ver =9.8
        if(evt_version.ge.9.8)then
           do i=44,46  !  _snw.tb0,  _drn.tb0,  _dsn.tb0
            read(99,99005,iostat=ios)junk,fln(i)
            if(iopt.eq.2.or.ios.ne.0)print*,junk,i,'   ',fln(i)
            if(numa.le.0)write(51,1030)i+250,i,fln(i)
          end do
...
c     add snow,drain,dsnow  ver =9.8
        if(evt_version.ge.9.8)then
           do i=47,49  !  _snw.r2c,  _drn.r2c,  _dsn.r2c
            read(99,99005,iostat=ios)junk,fln(i)
            if(iopt.eq.2.or.ios.ne.0)print*,junk,i,'   ',fln(i)
            if(numa.le.0)write(51,1030)i+250,i,fln(i)
          end do
...
!         will be nch+1
          ...
          do n=2,ni
            ...
            if(numa.le.0)write(51,1300)fln(100+n)
            if(ios.ne.0)then
