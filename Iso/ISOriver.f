...
! Called hourly or sub-hourly dpending on the routing time step.
!
!
!************************************************************************
...
!            UPDATE THE TRACER MASSES IN CHANNEL
     ...
     *         (1.0+qo2(n)*t/2.0/store2(n))
	       if(isoSTRstore2(n).lt.0.0) isoSTRstore2(n)=0.0000001
...
!           UPDATE MASS BEING INPUT INTO NEXT GRID CELL
            if(jz.ge.kt.and.nopt(l)==2.and.n==iflowgrid(l))then
